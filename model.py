...
class GardenModel():
  class Player(pygame.sprite.Sprite):
      ...
      def __init__(self, pos):
          ...
          """
          super().__init__()
          image = pygame.image.load("player_character.png").convert_alpha()
      ...
      def setup_level(self, level_map):
          ...
          for row_index, row in enumerate(level_map):
              for col_index, col in enumerate(row):
                  ...
                  if col == "v":
                    self.vert_border = GardenModel.Tile((x, y), "fence-vertical.png", (32, 45))
                    self.all_tiles.add(self.vert_border)
                  if col == "h":
                    self.hor_border = GardenModel.Tile((x, y), "fence-horizontal.png", (32, 45))
                    self.all_tiles.add(self.hor_border)
                  if col == "r":
                    self.right_end_border = GardenModel.Tile((x, y), "fence-right-end.png", (32, 45))
                    self.all_tiles.add(self.right_end_border)
                  if col == "T":
                    self.tree = GardenModel.Tile((x, y), "pom tree.png", (90, 90))
